...
PAIRS = [("БА","ПА"),("БО","ПО"),("БУ","ПУ"),("БЫ","ПЫ"),("БЭ","ПЭ"),
...
def main():
    used = {w.lower() for p in PAIRS for w in p}
    rows = ['<h2>Пары — слушайте подряд</h2>'
            '<p class="how">Слева Б, справа П. Включите оба подряд: если разница слышна '
            'сразу, слог годится. Если оба звучат одинаково — скажите, перезапишем.</p>']
    for a, b in PAIRS:
    ...
    open("poslushat-bp.html", "w", encoding="utf-8").write(
        HTML.replace("{{ROWS}}", "\n".join(rows)).replace("{{N}}", str(len(new))))
    print(f"poslushat-bp.html готов: {len(new)} фраз, {len(PAIRS)} пар")
...
<title>Котозарядка — послушать Б и П</title>
<style>
...
<main>
  <h1>Б и П — послушайте парами</h1>
  <p class="lead">{{N}} новых фраз, все вашим голосом. Б и П различаются одним:
     у Б связки включаются <b>до</b> взрыва, у П нет. Машиной я это проверить не смог —
     ухо надёжнее. Скажите, что перезаписать, или продиктуйте сами: для этой пары
     ваш живой голос точно лучше любого синтеза.</p>
  {{ROWS}}
</main>
</html>
"""
